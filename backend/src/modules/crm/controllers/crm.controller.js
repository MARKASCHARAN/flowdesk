...
export const crmController = {
  ...
  async getCustomers(req, res, next) {
    try {
      ...
      const { customers, total } = await crmService.getCustomers(
        res.locals.tenantId,
        { skip, take: limit, search: req.query.search }
      );
  ...
  async getCustomerById(req, res, next) {
    try {
      const customer = await crmService.getCustomerById(
        req.params.id,
        res.locals.tenantId
      );
  ...
  async getLeads(req, res, next) {
    try {
      ...
      const { leads, total } = await crmService.getLeads(res.locals.tenantId, {
        skip,
        take: limit,
  ...
  async addNoteToCustomer(req, res, next) {
    try {
      const note = await crmService.addNoteToCustomer(
        res.locals.tenantId,
        req.params.customerId,
        req.user.id,
        req.body.body
