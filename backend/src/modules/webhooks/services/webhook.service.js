...
export const webhookService = {
  ...
  async dispatchEvent(tenantId, eventType, payload) {
    try {
      ...
      const mockWebhookUrl = process.env.MOCK_WEBHOOK_URL || 'https://webhook.site/mock-endpoint';
      const mockWebhookSecret = process.env.MOCK_WEBHOOK_SECRET || 'whsec_mock123';

      logger.info(`[WebhookService] Queuing ${eventType} for Tenant: ${tenantId}`);
      ...
      logger.error(`[WebhookService] Error dispatching event: ${error.message}`);
    }
  }
};
